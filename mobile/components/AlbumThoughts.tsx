    ...
    <>
      ...
      >
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          {/* An invitation, not a statistic. This is the only way into the
              record's conversation now that the review cell is gone, so the
              line that leads has to be the thing you can do. */}
          <Text style={styles.title}>
            {open ? 'Join the discussion' : 'Notes open when you finish rating'}
          </Text>
          <Text style={styles.body} numberOfLines={2}>
            {!open
              ? 'Rate every track and the album to read and post here.'
              : count > 0
                ? `${count} ${count === 1 ? 'note' : 'notes'} from pressers who finished this record.`
                : 'Be the first to say something about this record.'}
          </Text>
        </View>
